...
// ║   Fixed: UTF-8 column widths, timing, status decay, warp overlay,        ║
// ║           scls/cout interleave, visLen for block chars                  ║
// ╚═══════════════════════════════════════════════════════════════════════════╝
...
static int visLen(const std::string& s) {
    ...
    while (i < s.size()) {
        ...
        if (c == '\033') {
            inEsc = true; ++i; continue;
        }
        if (inEsc) {
            if (std::isalpha(c)) inEsc = false;
            ++i; continue;
        }
        if ((c & 0xC0) != 0x80) {
            ++len;
...
static std::string padR(const std::string& s, int w) {
    ...
    if (vl > w) {
        ...
        bool inEsc = false;
        size_t i = 0;
        while (i < s.size() && vis < w - 3) {
            unsigned char c = (unsigned char)s[i];
            if (c == '\033') { inEsc = true; out += s[i++]; continue; }
            if (inEsc) { out += s[i]; if (std::isalpha(c)) inEsc = false; ++i; continue; }
            if ((c & 0xC0) != 0x80) ++vis;
            out += s[i++];
        }
        out += "...";
...
static std::string boxSide(const std::string& content, int w) {
    ...
    if (vl <= avail) {
        ...
        bool inEsc = false;
        size_t i = 0;
        while (i < content.size() && vis < avail) {
            unsigned char c = (unsigned char)content[i];
            if (c == '\033') { inEsc = true; inner += content[i++]; continue; }
            if (inEsc) { inner += content[i]; if(std::isalpha(c)) inEsc=false; ++i; continue; }
            if ((c & 0xC0) != 0x80) ++vis;
            inner += content[i++];
        }
        inner += A::R;
...
public:
    ...
    bool deleteCurrent() {
        ...
        v->prev->next=v->next;
        v->next->prev=v->prev;
        delete v; --size_; elapsedMs_=0; sessionListenMs_=0;
        return true;
    }
    ...
    void calculateStats() { calculateListeningStats(); }
...
private:
    ...
    std::vector<std::pair<std::string, int>> topArtists_;
    std::vector<std::pair<std::string, int>> topSongs_;
    std::vector<std::pair<std::string, int>> topGenres_;
    int totalMinutes_ = 0;
    ...
    void calculateListeningStats() {
        topArtists_.clear(); topSongs_.clear(); topGenres_.clear();
        std::map<std::string, int> artistPlays, songPlays, genrePlays;
        int totalPlays=0, totalRating=0, ratedSongs=0;
        int totalListenSeconds=(int)(totalListenMs_/1000);

        if(head_) {
            Node* n=head_;
            do {
                if(!n->song.artist.empty()){
                    artistPlays[n->song.artist]+=n->song.playCount;
                    totalPlays+=n->song.playCount;
                }
                if(!n->song.title.empty()){
                    std::string k=n->song.title+" - "+n->song.artist;
                    songPlays[k]+=n->song.playCount;
                }
                if(!n->song.genre.empty()) genrePlays[n->song.genre]+=n->song.playCount;
                if(n->song.rating>0){ totalRating+=n->song.rating; ratedSongs++; }
                n=n->next;
            } while(n!=head_);
        }

        for(const auto& p:artistPlays) topArtists_.push_back({p.first,p.second});
        std::sort(topArtists_.begin(),topArtists_.end(),[](const auto&a,const auto&b){return a.second>b.second;});
        if(topArtists_.size()>5) topArtists_.resize(5);

        for(const auto& p:songPlays) topSongs_.push_back({p.first,p.second});
        std::sort(topSongs_.begin(),topSongs_.end(),[](const auto&a,const auto&b){return a.second>b.second;});
        if(topSongs_.size()>5) topSongs_.resize(5);

        for(const auto& p:genrePlays) topGenres_.push_back({p.first,p.second});
        std::sort(topGenres_.begin(),topGenres_.end(),[](const auto&a,const auto&b){return a.second>b.second;});
        if(topGenres_.size()>5) topGenres_.resize(5);

        totalMinutes_=totalListenSeconds/60;
        avgPlaysPerSong_=size_>0?(double)totalPlays/size_:0;
        avgRating_=ratedSongs>0?(double)totalRating/ratedSongs:0;
        determineListeningVibe(artistPlays.size(),genrePlays.size(),avgPlaysPerSong_,avgRating_);
    }

    void determineListeningVibe(size_t uniqueArtists, size_t uniqueGenres, double avgPlays, double avgRating) {
        if(size_==0){ listeningVibe_="The Silent Listener - Time to add some music!"; return; }
        double ad=(double)uniqueArtists/size_;
        double gd=(double)uniqueGenres/size_;
        if(ad>0.7&&gd>0.5)       listeningVibe_="The Explorer - You love discovering new artists!";
        else if(avgPlays>5.0)     listeningVibe_="The Devotee - You have your favorites!";
        else if(avgRating>4.0)    listeningVibe_="The Connoisseur - Only the best of the best!";
        else if(totalMinutes_>10000) listeningVibe_="The Audiophile - Music is life!";
        else if(uniqueGenres<=2)  listeningVibe_="The Specialist - You know what you like!";
        else {
            std::vector<std::string> v={
                "The Rocker - Keeping the spirit alive!",
                "The Virtuoso - Appreciating craftsmanship!",
                "The Night Owl - Best listening after dark!",
                "The Scholar - Exploring music's rich history!",
...
private:
    ...
    static constexpr double STATUS_TTL_SEC = 4.0;
    int          cols_=80, rows_=24;
    Mode         mode_ = Mode::MAIN;
    std::string  warpInput_;

    bool statusExpired() {
        if(statusMsg_.empty()) return true;
        struct timespec now;
        clock_gettime(CLOCK_MONOTONIC, &now);
        double elapsed = (now.tv_sec - statusSetAt_.tv_sec)
                       + (now.tv_nsec - statusSetAt_.tv_nsec) * 1e-9;
        return elapsed >= STATUS_TTL_SEC;
    }
    ...
    void drawWarpMode() {
        ...
        // Box dimensions — keep it narrower than terminal, at most 72 cols
        int bw = std::min(72, cols_ - 4);
        if (bw < 40) bw = 40;
        int bh = 22;

        // Center the overlay using real terminal dimensions
        int r0 = (rows_ - bh) / 2;
        int c0 = (cols_ - bw) / 2;
        if (r0 < 1)  r0 = 1;
        if (c0 < 1)  c0 = 1;

        // Helper: emit a box row at (row, col)
        auto emit = [&](int row, const std::string& content) {
            std::cout << A::go(row, c0) << A::MG << content << A::R;
        };

        int row = r0;
        emit(row++, boxTop(bw));
        emit(row++, boxSide(std::string(A::BW)+"  PHANTOM WRAPPED -- YOUR LISTENING STATS",bw));
        emit(row++, boxSide("",bw));

        // Top Artists
        emit(row++, boxSide(std::string(A::W)+"  TOP ARTISTS",bw));
        {
            const auto& v=mp_.getTopArtists();
            if(v.empty()){
                emit(row++, boxSide(std::string(A::MG)+"  No play data yet -- start listening!",bw));
            } else {
                for(size_t i=0;i<v.size()&&i<3;i++){
                    std::string s="  "+std::to_string(i+1)+". "+padR(v[i].first,24)
                                  +" "+std::to_string(v[i].second)+" plays";
                    emit(row++, boxSide(std::string(A::LG)+s,bw));
                }
            }
        }
        emit(row++, boxSide("",bw));
        ...
        // Top Songs
        emit(row++, boxSide(std::string(A::W)+"  TOP SONGS",bw));
        {
            const auto& v=mp_.getTopSongs();
            if(v.empty()){
                emit(row++, boxSide(std::string(A::MG)+"  No play data yet",bw));
            } else {
                for(size_t i=0;i<v.size()&&i<3;i++){
                    std::string s="  "+std::to_string(i+1)+". "+padR(v[i].first,30)
                                  +" "+std::to_string(v[i].second)+"x";
                    emit(row++, boxSide(std::string(A::LG)+s,bw));
                }
            }
        }
        ...
        emit(row++, boxSide(std::string(A::W)+"  TOP GENRES",bw));
        {
            const auto& v=mp_.getTopGenres();
            if(v.empty()){
                emit(row++, boxSide(std::string(A::MG)+"  No data yet",bw));
            } else {
                int maxP=v[0].second>0?v[0].second:1;
                int barW=std::min(30, bw-30);
                if(barW<4) barW=4;
                for(size_t i=0;i<v.size()&&i<3;i++){
                    int filled=(v[i].second*barW)/maxP;
                    std::string s="  "+padR(v[i].first,14)
                                  +rep("|",filled)+rep("-",barW-filled)
        ...
        // Vibe + extra stats
        emit(row++, boxSide(std::string(A::W)+"  LISTENING PERSONALITY",bw));
        emit(row++, boxSide(std::string(A::BW)+"  "+mp_.getListeningVibe(),bw));
        {
            char buf[64];
            std::snprintf(buf,sizeof(buf),"  Avg plays/song: %.1f  |  Avg rating: %.1f/5",
                          mp_.getAvgPlaysPerSong(), mp_.getAvgRating());
            emit(row++, boxSide(std::string(A::LG)+buf,bw));
        }
        emit(row++, boxSide("",bw));

        // Seek input
        emit(row++, boxSide(std::string(A::LG)+"  Seek to % (0-100):  "
                            +std::string(A::W)+warpInput_+"_",bw));
        emit(row++, boxSide(std::string(A::DIM)+"  Enter=jump  Esc=cancel",bw));
        emit(row,   boxBot(bw));
    ...
    void drawNowPlaying() {
        ...
        if(!s){
            ...
            int titleW = cols_ - 4 - 3 - (int)elStr.size() - 1;
        ...
        if(s){
            ...
            std::cout << "  " << stars;
            int used2 = 2 + 2 + 22 + 14 + (int)yearStr.size() + 2 + visLen(stars);
            int tagSpace = cols_ - 4 - used2;
            if(!s->tags.empty() && tagSpace > 5){
                std::string tagStr = "  #"+s->tags;
                if(visLen(tagStr) > tagSpace) tagStr = padR(tagStr, tagSpace);
    ...
    void drawTransportRow() {
        ...
        if(!statusMsg_.empty() && statusW > 0){
            if(statusExpired()){
                statusMsg_.clear();
                std::cout << std::string((size_t)statusW,' ');
            } else {
                std::string disp;
                int vis = 0;
                bool inEsc = false;
                size_t i = 0;
                while(i < statusMsg_.size() && vis < statusW){
                    unsigned char c=(unsigned char)statusMsg_[i];
                    if(c=='\033'){inEsc=true; disp+=statusMsg_[i++]; continue;}
                    if(inEsc){disp+=statusMsg_[i]; if(std::isalpha(c)) inEsc=false; ++i; continue;}
                    if((c&0xC0)!=0x80) ++vis;
                    disp+=statusMsg_[i++];
                }
                if(inEsc) disp += A::R;
    ...
    void drawPlaylist() {
        ...
        std::cout << padR("DUR",7) << "STARS  PLAYS\n" << A::R;
        line(cols_);
    ...
    void drawFooterBar() {
        std::cout << A::MG << "╠" << rep("═",cols_-2) << "╣\n" << A::R;
        std::string keys="  SPC:play  <->:skip  ^v:vol  r:rpt  s:shuf  w:warp+stats  f/B:seek  b:bmark  m:rate  a:add  /:search  ?:help  q:quit";
