// ╔═══════════════════════════════════════════════════════════════════════════╗
// ║   PHANTOM  —  Terminal Music Player  v3.0  (Linux, C++17)               ║
// ║   Circular Doubly Linked List · Raw TTY · CSV · Real-time TUI           ║
// ╚═══════════════════════════════════════════════════════════════════════════╝
//
//  Build:  g++ -std=c++17 -O2 -Wall -o phantom music_player.cpp
//  Run:    ./phantom  [playlist.csv]
//
//  ┌─ KEYS ──────────────────────────────────────────────────────────────────┐
//  │  SPACE        play / pause          q        quit & save                │
//  │  ← →          prev / next           ↑ ↓      volume                     │
//  │  n / p        next / prev (vi)      + / -    volume fine                │
//  │  r            repeat cycle          s        shuffle                     │
//  │  z            random jump           b        bookmark current time       │
//  │  f            seek forward 10s      B        seek back 10s              │
//  │  a            add song              d        delete current              │
//  │  e            edit song             i        song details                │
//  │  /            search                l        playlist browser            │
//  │  h            history               t        stats dashboard            │
//  │  S            sort menu             w        warp (time-jump) mode      │
//  │  c            create playlist       g        genre filter               │
//  │  m            mark / rate song      ?        help                       │
//  └─────────────────────────────────────────────────────────────────────────┘
...
static std::string readLine(const std::string& defaultVal = "") {
    if (!gRaw) { std::string s; std::getline(std::cin,s); return s.empty()?defaultVal:s; }
...
static std::string fmtTimeLong(int sec) {
    char b[32]; std::snprintf(b,sizeof(b),"%dh %02dm %02ds",sec/3600,(sec%3600)/60,sec%60); return b;
}
static int parseSecs(const std::string& dur) {
    int m=0,s=0; std::sscanf(dur.c_str(),"%d:%d",&m,&s); return m*60+s;
}
...
// Waveform-style animated visualizer bar (purely ASCII art, changes per tick)
static std::string waveBar(int tick, int width, bool playing) {
    if (!playing || width < 2) {
        std::string s(width, '-'); return s;
    }
    // Simulate a quasi-random waveform based on tick
    std::string bar;
    bar.reserve(width * 3);
    const char* blocks[] = {" ","▁","▂","▃","▄","▅","▆","▇","█"};
    for (int i = 0; i < width; i++) {
        ...
        v = (v + 1.0f) * 0.5f; // 0..1
        int idx = (int)(v * 8); if(idx>8)idx=8; if(idx<0)idx=0;
        bar += blocks[idx];
    }
    return bar;
}

// Filled progress bar using block characters
static std::string pbar(float fill, int width) {
    ...
    for(int i=0;i<width;i++) {
        ...
        else if(i==filled){ bar+="\xe2\x96\x8c"; } // ▌  (half block leading edge)
...
static std::string vbar(int vol, int w) {
    ...
    std::string b;
    for(int i=0;i<w;i++) b+=(i<f?"\xe2\x96\x88":"\xe2\x96\x91");
    return b;
}
...
static std::string starStr(int rating) {
    std::string s;
    for(int i=1;i<=5;i++) s += (i<=rating ? "\xe2\x98\x85" : "\xe2\x98\x86");
    return s;
}
...
static std::string boxSide(const std::string& content, int w) {
    // pad content to w-4 then wrap in ║ ··· ║
...
struct HistEntry {
    std::string title;
    std::string artist;
    std::string playedAt; // HH:MM:SS
    int         listenSec = 0; // how many seconds we actually listened
};
...
public:
    ...
    void loadBookmarks(const std::string& file) {
        ...
        while(std::getline(f,line)){
            ...
            Bookmark b;
            b.songTitle=flds[0]; b.songArtist=flds[1];
    ...
    bool deleteCurrent() {
        ...
        cur_=v->next; if(v==head_) head_=v->next;
        v->prev->next=v->next; v->next->prev=v->prev;
        delete v; --size_; elapsed_=0; return true;
    }
    ...
    void seekSec(int delta) {
        ...
        if(elapsed_<0) elapsed_=0;
        if(elapsed_>maxMs) elapsed_=maxMs-1000;
    }
    // Warp: jump to absolute percentage of song
    void warpTo(float pct) {
        if(!cur_) return;
        int maxMs = cur_->song.toSecs()*1000;
        if(maxMs<=0) maxMs=30000;
        pct = std::max(0.0f,std::min(1.0f,pct));
        elapsed_ = (int)(pct*(float)maxMs);
    }

    // ── Tick ─────────────────────────────────────────────────────
    void advance(int ms=100) {
        if(!playing_||!cur_) return;
        elapsed_+=ms; listenMs_+=ms; tick_++;
        ...
        if(elapsed_>=total){
            ...
            pushHistory();
            switch(repeat_){
                case RepeatMode::ONE: elapsed_=0; break;
                case RepeatMode::ALL:
                    cur_=cur_->next; elapsed_=0; break;
    ...
    void addBookmark(const std::string& label) {
        ...
        b.positionSec=elapsed_/1000; b.label=label;
        bookmarks_.push_back(b);
    ...
    void jumpToBookmark(int idx) {
        ...
        if(n){ cur_=n; elapsed_=b.positionSec*1000; playing_=true; }
    }
    ...
    int  sessionListenSec() const { return (int)(listenMs_/1000); }
    ...
    int   elapsedMs()          const { return elapsed_; }
    float progress()           const {
        if(!cur_) return 0.f;
        int t=cur_->song.toSecs(); return t>0?std::min(1.0f,(float)elapsed_/((float)t*1000.f)):0.f;
    }
...
private:
    ...
    int        dupesOnLoad_ = 0;
    int        tick_     = 0;
    int64_t    listenMs_ = 0;    // this session
    int64_t    totalListenMs_=0; // all time (not persisted yet, per-session)
    int        listenSecAtSongStart_ = 0;
    std::string today_;
    std::deque<HistEntry> histQ_;   // last 50
    ...
    void pushHistory() {
        ...
        HistEntry e; e.title=cur_->song.title; e.artist=cur_->song.artist;
        e.playedAt=buf; e.listenSec=elapsed_/1000;
        if(histQ_.size()>=50) histQ_.pop_front();
        histQ_.push_back(e);
    }
    void commitListen() {
        // nothing to persist inline; stats tracked per tick
    }
    ...
    void freeList() {
        if(!head_) return;
        Node* n=head_; do{ Node* t=n->next; delete n; n=t; }while(n!=head_);
    ...
    static std::string ce(const std::string& v) {
        ...
        std::string r="\""; for(char c:v){if(c=='"')r+='"';r+=c;} return r+'"';
    }
};
...
public:
    ...
    // Warp mode: user types a percentage
    ...
    void modalSearch() {
        ...
        std::cout<<"\n";
        if(results.empty()){ std::cout<<A::MG<<"  No results.\n"<<A::R; }
        else {
            mline(cols_);
    ...
    void modalPlaylist() {
        ...
        while(true){
            scls();
            int visible=rows_-12; if(visible<3) visible=3;
            ...
            if(mp_.empty()){ std::cout<<A::MG<<"  Empty.\n"<<A::R; }
            else {
            ...
            else if(k==27){ int k2=readKey(); if(k2=='['){ int k3=readKey();
                if(k3=='A'&&offset>0)--offset; else if(k3=='B'&&offset+visible<total)++offset; }}
        }
    }
    ...
    void modalHistory() {
        ...
        if(h.empty()){ std::cout<<A::MG<<"  No history.\n"<<A::R; }
        else {
            mline(cols_);
    ...
    void modalStats() {
        ...
        for(auto& kv:gmap){
            ...
            std::cout<<A::MG<<"  "<<padR(kv.first,16)
                     <<A::W<<rep("\xe2\x96\x88",filled)
                     <<A::MG<<rep("\xe2\x96\x91",barW-filled)
                     <<" "<<kv.second<<A::R<<"\n";
        }
        waitPrompt();
    }
    ...
    void modalGenreFilter() {
        ...
        if(genres.empty()){ std::cout<<A::MG<<"  No genres.\n"<<A::R; waitPrompt(); return; }
        std::cout<<"\n";
    ...
    void modalBookmarks() {
        ...
        if(bms.empty()){ std::cout<<A::MG<<"  No bookmarks.\n"<<A::R; }
        else {
            mline(cols_);
        ...
        if(cmd=="a"||cmd=="A"){
            ...
            mp_.saveCSV(); // bookmarks embedded in session
        } else if(cmd[0]=='d'){
    ...
    void modalRate() {
        ...
        try{ mp_.rateCurrent(std::stoi(r)); mp_.saveCSV(); }
        catch(...){ setStatus("[!] Invalid rating."); }
    }
...
private:
    ...
    void mline(int c, char ch='-')   { std::cout<<A::MG<<std::string((size_t)c,ch)<<A::R<<"\n"; }
    void mlineB(int c, char ch='=')  { std::cout<<A::MG<<std::string((size_t)c,ch)<<A::R<<"\n"; }
    void eline(int c, char ch='-')   { emit(A::MG); emit(std::string((size_t)c,ch)); emit(A::R); emit("\n"); }
    void elineB(int c, char ch='=')  { emit(A::MG); emit(std::string((size_t)c,ch)); emit(A::R); emit("\n"); }
    ...
    // ── WARP mode overlay ─────────────────────────────────────────
    void drawWarpMode() {
        drawMain(); // draw main as bg
        // Overlay a centered box
        int bw=40, bh=7;
        int r0=(rows_-bh)/2, c0=(cols_-bw)/2;
        // Build box into buf
        // Since buf already has main, append cursor-addressed overlay
        emit(A::go(r0,c0)); emit(A::MG); emit(boxTop(bw)); emit(A::R);
        emit(A::go(r0+1,c0)); emit(A::MG); emit(boxSide(A::BW+std::string("  WARP TO POSITION")+A::MG,bw)); emit(A::R);
        emit(A::go(r0+2,c0)); emit(A::MG); emit(boxSide("",bw)); emit(A::R);
        emit(A::go(r0+3,c0)); emit(A::MG);
        emit(boxSide(std::string(A::LG)+"  Enter %  (0-100):  "+std::string(A::W)+warpInput_+std::string(A::MG)+"_",bw)); emit(A::R);
        emit(A::go(r0+4,c0)); emit(A::MG); emit(boxSide("",bw)); emit(A::R);
        emit(A::go(r0+5,c0)); emit(A::MG); emit(boxSide(std::string(A::DIM)+"  Enter=jump  Esc=cancel",bw)); emit(A::R);
        emit(A::go(r0+6,c0)); emit(A::MG); emit(boxBot(bw)); emit(A::R);
    }
    ...
    // Top bar: app name + date/time + track counter
    void drawTopBar() {
        // Left: PHANTOM logo text
        emit(A::MG); emit("╔"); emit(rep("═",cols_-2)); emit("╗\n"); emit(A::R);

        // App name left, date centre, track right
        ...
        emit(A::MG); emit("║"); emit(A::R);
        emit(A::BW);  emit(appName);
        emit(std::string((size_t)gap1,' '));
        emit(A::DG);  emit(dateStr);
        emit(std::string((size_t)gap2,' '));
        emit(A::MG);  emit(trkStr);
        emit(A::MG);  emit("║\n"); emit(A::R);
    ...
    void drawNowPlaying() {
        const Song* s=mp_.currentSong();
        // Two lines: title line + meta line

        // ── Title line
        emit(A::MG); emit("║ "); emit(A::R);
        if(!s){
            emit(A::MG); emit(padR("  No song loaded  — press [a] to add",cols_-4)); emit(A::R);
        } else {
            std::string icon = mp_.isPlaying()
                ? std::string(A::BW)+"▶ "
                : std::string(A::MG)+"‖ ";
            emit(icon);
            // Title (bold white), truncated
            int titleW=cols_-6-(int)fmtTime(mp_.elapsedSec()).size()-3;
            emit(A::BW); emit(padR(s->title,titleW)); emit(A::R);
        }
        emit(A::MG); emit(" ║\n"); emit(A::R);
        ...
        emit(A::MG); emit("║ "); emit(A::R);
        if(s){
            emit(A::MG); emit("  "); // indent past icon
            emit(A::LG); emit(padR(s->artist,22));
            emit(A::MG); emit(padR(s->genre,14));
            emit(s->year>0?std::to_string(s->year):"");
            emit("  "); emit(starStr(s->rating));
            emit(A::MG);
            // Right-align tags if any
            if(!s->tags.empty()){
                ...
                emit(A::MG); emit(tagStr);
            }
        } else {
            emit(std::string((size_t)(cols_-4),' '));
        }
        emit(A::R);
        // Pad to width
        emit(A::MG); emit(" ║\n"); emit(A::R);
    ...
    void drawVisualizer() {
        ...
        emit(A::MG); emit("║ ");
        emit(A::DIM); emit(A::W); emit(wave); emit(A::R);
        emit(A::MG); emit(" ║\n"); emit(A::R);
    }
    ...
    void drawProgressRow() {
        ...
        emit(A::MG); emit("║ ");
        emit(A::MG); emit(elStr); emit("  ");
        emit(A::DIM); emit(A::W); emit(bar); emit(A::R);
        emit(A::MG); emit("  "); emit(totStr);
        emit(A::MG); emit(" ║\n"); emit(A::R);
    }

    void drawTransportRow() {
        // VOL | REPEAT | STATUS MSG
        ...
        emit(A::MG); emit("║ ");
        emit(A::MG); emit("VOL["); emit(A::W); emit(volStr); emit(A::MG); emit("] ");
        emit(A::DG); emit(volPct); emit("  ");
        emit(A::MG); emit(rep); emit("  ");

        // Status message (auto-clears after ~30 frames)
        if(!statusMsg_.empty()){
            emit(A::W); emit(statusMsg_); emit(A::R);
    ...
    void drawPlaylist() {
        ...
        emit(A::MG);
        emit("  "); emit(padR("#",5));
        emit(padR("TITLE",28)); emit(padR("ARTIST",20));
        emit(padR("DUR",7)); emit("★  PLAYS\n"); emit(A::R);
        eline(cols_);

        if(mp_.empty()){
            emit(A::MG); emit("  Playlist is empty. Press [a] to add songs.\n"); emit(A::R);
            return;
        }
        // How many rows
        int used=14; // top bars + footer
        int available=rows_-used; if(available<2)available=2;
        ...
        do {
            if(idx>=offset&&idx<offset+available){
                bool ic=(idx==curIdx);
                // Alternate dim rows for readability
                std::string numStr=padR(std::to_string(idx+1)+".",5);
                std::string row="  "+numStr+padR(n->song.title,28)+padR(n->song.artist,20)
                               +padR(n->song.duration,7)+std::to_string(n->song.rating)
                               +"  "+std::to_string(n->song.playCount);
                if(ic){ emit(A::INV); emit(row); emit(A::R); emit("\n"); }
                else if(idx%2==0){ emit(A::LG); emit(row); emit(A::R); emit("\n"); }
                else  { emit(A::MG); emit(row); emit(A::R); emit("\n"); }
            }
            n=n->next; ++idx;
        } while(n!=mp_.headNode());
        ...
        emit(A::MG); emit("  ");
        emit(std::to_string(total)+" songs  |  #"+std::to_string(curIdx+1));
        emit("  "+std::to_string(pct)+"%\n"); emit(A::R);
    }
    ...
    void drawFooterBar() {
        ...
        emit(A::MG); emit("║"); emit(A::DG); emit(padR(keys,cols_-2)); emit(A::MG); emit("║\n"); emit(A::R);
        emit(A::MG); emit("╚"); emit(rep("═",cols_-2)); emit("╝\n"); emit(A::R);
    }
};
...
int main(int argc, char* argv[]) {
    ...
    while(!quit){
        ...
        switch(k){
            ...
            case '+': case '=':
                mp.volUp(); ui.setStatus("Vol "+std::to_string(mp.volume())+"%"); break;
            case '-': case '_':
                mp.volDown(); ui.setStatus("Vol "+std::to_string(mp.volume())+"%"); break;

            case 'r': case 'R':
                mp.cycleRepeat();
                ui.setStatus(std::string("Repeat: ")+rLabel(mp.repeat())); break;

            case 's':
                mp.shuffle(); ui.setStatus("Shuffled!"); break;

            case 'z': case 'Z':
                mp.jumpRandom(); break;

            case 'f':
                mp.seekSec(10); ui.setStatus("Seek +10s"); break;

            case 'B':
                mp.seekSec(-10); ui.setStatus("Seek -10s"); break;

            case 'w': case 'W':
                ui.setMode(UI::Mode::WARP); break;

            case 'b':
                // Quick bookmark
                mp.addBookmark("bm@"+fmtTime(mp.elapsedSec()));
                mp.saveBookmarks(BM_FILE); break;
            ...
            case '/':
                ui.modalSearch(); break;

            case 'l': case 'L':
                ui.modalPlaylist(); break;

            case 'h': case 'H':
                ui.modalHistory(); break;
